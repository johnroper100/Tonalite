...
last = 0
data = None
with open('CSEDIT_IMAGES.dat', 'rb') as f:
    data = bytearray(f.read())
with open('CSEDIT_IMAGES.idx', 'r') as ff:
    lines = ff.readlines()
    for l in range(1, len(lines)):
        li = lines[l].split(",")
        lr = lines[l-1].split(",")
...
for image in images:
    if not os.path.exists('../fixture-images/'+image[0]+".png"):
        with open('../fixture-images/'+image[0]+".png", 'wb') as f:
            f.write(bytes(image[2]))
    else:
        print('../fixture-images/'+image[0]+".png")